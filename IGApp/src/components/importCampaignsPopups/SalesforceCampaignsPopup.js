...
export default class SalesforceCampaigns extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const customCampaignTypeField = get(nextProps.data, ['campaignsMapping', 'customCampaignTypeField'], ...this.getDefaultCustomCampaignTypeField());
    this.setState({customCampaignTypeField});
  }

  getDefaultCustomCampaignTypeField = () => {
    return [{
      value: 'Type',
      label: 'Type'
    }];
  ...
  getCampaignTypeCustomField = (code) => {
    return new Promise((resolve, reject) => {
      serverCommunication.serverRequest('PATCH',
        ...
        .then(response => {
          if (response.ok) {
            response.json()
              .then(data => {
                const customFieldsCampaignTypes = [...this.getDefaultCustomCampaignTypeField(), ...data];
                this.setState({customFieldsCampaignTypes});
                resolve();
              });
          }
            ...
            reject(`Error getting salesforce campaign type custom fields`);
          }
        })
        .catch(err => {
          reject(err);
  ...
  validateSelected = () => {
    const isEmpty = Object.keys(this.refs).some(ref => {
      ...
      else if (ref.includes(statusRef) && !this.refs[ref].props.selected) {
        this.refs[ref].focus();
        return true;
      }
      return false;
  ...
  nextStep = () => this.state.deferred.promise;

  next = () => {
    const campaignsMapping = {...this.state.campaignsMapping};
    const customCampaignTypeFieldValue = get(this.state.customCampaignTypeField, ['value'], this.state.customCampaignTypeField);
    campaignsMapping.customCampaignTypeField = customCampaignTypeFieldValue;
    this.setState({campaignsMapping}, () => this.state.deferred.resolve(customCampaignTypeFieldValue));
  };

  handleChangeCustomField = customCampaignTypeField => {
    this.setState({customCampaignTypeField});
  };
  ...
  render() {
    ...
    return <AuthorizationIntegrationPopup ref={ref => this.authPopup = ref}
                                          ...
                                          afterAuthorizationBeforeMappingStep={this.getCampaignTypeCustomField}
                                          haveStepAfterAuthorizationBeforeMapping={true}
                                          ...
                                          fieldKey='customCampaignTypeFieldValue'
                                          platformTitle='Salesforce'>
      {this.state.tab === 0 ?
        <div>
          <Title title="SalesForce - Custom Fields"/>
          <div className={this.classes.row}>
            <Label style={{fontSize: '16px', marginBottom: 'inherit'}}>Choose a field for mapping a channel per
              campaign</Label>
            <div className={this.classes.row} style={{display: 'flex'}}>
              ...
              <div className={this.classes.colRight}>
                <Select select={{
                  options: this.state.customFieldsCampaignTypes
                }}
                        style={{width: '270px'}}
                        selected={this.state.customCampaignTypeField}
                        onChange={event => this.handleChangeCustomField(event.value)}/>
              </div>
            </div>
          </div>
        </div>
