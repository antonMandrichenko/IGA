...
class AppComponent extends Component {
  ...
  getUserMonthPlan(region, planDate) {
    return this.getUserMonthPlanWithOptions(region, planDate, 'usermonthplan-noattr');
  }
  ...
  runScheduleTask() {
    ...
    const deferred = q.defer();
    serverCommunication.serverRequest('POST', 'runScheduleTask', null, localStorage.getItem('region'))
      .then(response => {
        if (response.ok) {
