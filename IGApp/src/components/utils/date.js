...
export const getStartOfDayTimestamp = (date) => new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
export const getStartOfWeekTimestamp = (date) => {
  const day = date.getDay();
  const diff = date.getDate() - day + (day === 0 ? -6 : 1); // adjust when day is sunday
  const mondayDate = new Date(date.setDate(diff));

  return getStartOfDayTimestamp(mondayDate);
}
