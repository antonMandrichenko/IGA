...
export default class CMO extends Component {
  ...
  getMonthlyDataByQuarter = (indicatorsData) => {
    const {
      actualIndicators,
      calculatedData: {
        lastYearHistoryData: {
    ...
    const monthlyDataWithDate = indicatorsData.map((data, index) => ({
      ...data,
      name: months[index]
    }));

    // get month names in current year
    const currentYear = new Date().getMonth() < fiscalYearFirstMonth ? moment(new Date()).format("YY") -1 : moment(new Date()).format("YY");
    let currentYearMonths = [];
    let month = fiscalYearFirstMonth;
    do {
      if (month < fiscalYearFirstMonth) {
        currentYearMonths.push(moment().year(Number(currentYear) + 1).month(month).format("MMM YY"));
      }
      else {
        currentYearMonths.push(moment().year(currentYear).month(month).format("MMM YY"));
      }
      month = (month + 1) % 12;
    } while (month !== fiscalYearFirstMonth);

    // filter months based on quarter
    const currentMonth = (new Date()).getMonth();
    let currentQuarter;
    if (currentMonth < fiscalYearFirstMonth) {
      currentQuarter = Math.floor((currentMonth + 12 - fiscalYearFirstMonth) / 3);
    }
    else {
      currentQuarter = Math.floor((currentMonth - fiscalYearFirstMonth) / 3);
    }
    
    const currentQuarterStart = currentQuarter * 3;
    const currentQuarterMonthsNames = currentYearMonths.slice(currentQuarterStart, currentQuarterStart + 3);
    const currentQuarterMonthsData = monthlyDataWithDate.reduce((result, {name, ...data}) => {
    ...
    return currentQuarterMonthsData;
  };

  getMonthlyDataByYear = (indicatorsData) => {
    const {
      actualIndicators,
      calculatedData: {
        lastYearHistoryData: {
    ...
    const currentYear = new Date().getMonth() < fiscalYearFirstMonth ? moment(new Date()).format("YY") -1 : moment(new Date()).format("YY");
    let currentYearMonths = [];
    let month = fiscalYearFirstMonth % 12;
    do {
      if (month < fiscalYearFirstMonth) {
        currentYearMonths.push(moment().year(Number(currentYear) + 1).month(month).format("MMM YY"));
      }
    ...
    const currentYearMonthsData = monthlyDataWithDate.reduce((result, {name, ...data}) => {
      if (currentYearMonths.indexOf(name) !== -1) {
        result.push(data);
      }
      return result;
    }, []);

    return currentYearMonthsData;
  };
