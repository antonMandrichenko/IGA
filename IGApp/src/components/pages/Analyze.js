...
class Analyze extends Component {
  ...
  pullAttributionData = (newTimeFrame, newAttributionModel, newZeroWeightsNotInTimeFrame) => {
    const {
      attributionStore: {pullAttributionData, timeFrame, attributionModel, zeroWeightsNotInTimeFrame}
    } = this.props;

    pullAttributionData(newTimeFrame || timeFrame, newAttributionModel || attributionModel.value, newZeroWeightsNotInTimeFrame || zeroWeightsNotInTimeFrame);
  };
  ...
  handleChangeZeroWeightsNotInTimeFrame = ({value}) => {
    const {
      attributionStore: {setZeroWeightsNotInTimeFrame}
    } = this.props;

    setZeroWeightsNotInTimeFrame(value);

    this.pullAttributionData(null, null, value);
  };

  handleChangeAttributionModel = model => {
    const {
      attributionStore: {setAttributionModel}
    } = this.props;

    setAttributionModel(model);

    this.pullAttributionData(null, model.value, null);
  };

  handleChangeTimeFrame = ({value}) => {
    const {

      attributionStore: {setTimeFrame}
    } = this.props;
          ...
          <div className={this.classes.contentHead}>
            ...
            <div className={analyzeStyle.locals.filter}>
              ...
              <div className={analyzeStyle.locals.filterItem}>
                Attribution Credit
                <Select
                  selected={zeroWeightsNotInTimeFrame}
                  select={{
                    options: [{value: false, label: 'All'}, {value: true, label: 'Time Frame'}]
                  }}
                  onChange={this.handleChangeZeroWeightsNotInTimeFrame}
                  className={analyzeStyle.locals.modelSelect}
                />
              </div>
            </div>
