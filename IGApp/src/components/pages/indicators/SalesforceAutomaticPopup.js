...
import CRMCustomField from 'components/pages/indicators/CRMCustomField';
...
export default class SalesforceAutomaticPopup extends Component {
  ...
  afterDataRetrieved = (data) => {
    return new Promise((resolve, reject) => {
      this.setState({
        ...
        stages: data.stages,
        owners: data.owners,
        fields: data.opportunity,
        customFields: {
          oppsCustomFields: data.opportunity,
          accountCustomFields: data.account,
          contactCustomFields: data.contact,
          leadsCustomFields: data.lead
        }
      });
      resolve(true);
    });
  ...
  mergeMapCustomFields = () => {
    const customFieldMapping = {...this.state.mappedCustomFields};
    const mappedCustomFields = {};
    Object.keys(customFieldMapping).forEach(key => {
      Object.keys(customFieldMapping[key]).forEach(field => {
        const oneMappedField = customFieldMapping[key][field];
        const customFieldKey = Object.keys(oneMappedField)[0];
        if (_.isEmpty(mappedCustomFields[customFieldKey])) {
          mappedCustomFields[customFieldKey] = [];
        }
        mappedCustomFields[customFieldKey].push(oneMappedField[customFieldKey]);
      });
    });
  ...
  getUserData = () => {
    ...
    const mappedCustomFields = this.mergeMapCustomFields();
    Object.keys(mappedCustomFields).forEach(element => {
      mapping[element] = mappedCustomFields[element];
    });
  ...
  handleAddCustomField = (key, object, field) => {
    let mappedCustomFields = {...this.state.mappedCustomFields};
    if (isEmpty(mappedCustomFields[key])) {
      mappedCustomFields[key] = [];
    }
    mappedCustomFields[key].push({[object]: field});
    this.setState({mappedCustomFields});
  };

  handleDeleteCustomField = (key, object) => {
    const mappedCustomFields = {...this.state.mappedCustomFields};
    mappedCustomFields[key] = mappedCustomFields[key].filter(obj => Object.keys(obj).pop() !== object);
    this.setState({mappedCustomFields});
  };
  ...
  render() {
    ...
    >
      ...
      <div className={this.classes.row} style={{marginBottom: '0px'}}>
        <Label className={salesForceStyle.locals.label}>
          import fields
        </Label>
      </div>
      {this.state.numOfLines ?
        this.getCRMCustomField()
        : null}
      <div className={this.classes.row} style={{display: 'flex'}}>
        <div className={salesForceStyle.locals.add}
             onClick={() => this.addLine()}>
        </div>
      </div>
      <div className={this.classes.row}>
        <div className={this.classes.row}>
          <Label checkbox={this.state.mapping.MRR !== undefined}
                 onChange={this.toggleCheckboxObject.bind(this, 'MRR')}
      ...
      <div className={this.classes.row}>
        <Label checkbox={this.state.mapping.ARPA !== undefined}
               onChange={this.toggleCheckboxObject.bind(this, 'ARPA')}
