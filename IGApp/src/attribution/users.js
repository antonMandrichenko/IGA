...
const calcPagesContent = (pages) => pages.reduce(
  (acc, curr) => {
    const fields = [
      ['title', 'uniqContent'],
...
const calcContentChannelsAndCampaigns = (sessions) => {
    const channels = [];
    const campaigns = [];

    sessions.forEach(({channel, campaign}) => {
    ...
    return {
        uniqContentChannels: uniqArr(channels),
        uniqContentCampaigns: uniqArr(campaigns),
    }
};
...
export const formatUsersReworked = (
  ...
  usersData.map(user => {
    ...
    const timeSinceFirst = timeSince(firstTouchPoint);
    const timeSinceLast = timeSince(lastTouchPoint);
    const channelsAndCampaigns = calcContentChannelsAndCampaigns(user.sessions);
    ...
    const channelBeforeStages = user.sessions
      ...
      .map(({ channel, pages, campaign, startTime }) => {
        const notReachedFunnelStages = [];
        const startDate = new Date(startTime);
        const pagesContent = calcPagesContent(pages);
        const isContent = isContentChannel(channel);

        if (isContent) {
            pagesContent.uniqContentTypes.push(channel);
            pagesContent.uniqContentPath.push(campaign);
            channel = null;
            campaign = null;
        }
    ...
    pagesContent.uniqContent.push(...channelsAndCampaigns.uniqContentCampaigns);
    user.uniqContent = pagesContent.uniqContent;
    pagesContent.uniqContentPath.push(...channelsAndCampaigns.uniqContentCampaigns);
    user.uniqContentPath = pagesContent.uniqContentPath;
    pagesContent.uniqContentTypes.push(...channelsAndCampaigns.uniqContentChannels);
